...
- 크로스-코어 캐시 간섭 항 `Ibar_i^{c,static}` 성격의 정적 간섭량 (HB/ECB/CB 집합 분석)
- 선점 지연 항 `CRPD̄_i^static` (UCB/ECB 데이터플로우 분석)
- 최종 복합 지표 `CAII_i^static = CA_i / (1 + Ibar + CRPD̄)`

초기 환경 검증용이던 결함 탐지(NullDeref 등)는 범위에서 제거되었다. 현재 목표는 **정적 캐시 분석 파이프라인 구축**에 집중하는 것이다.
...
10. `src/passes/InterferenceComputePass.cpp` — 집합별 LRU 충돌 분석, `I_i^{c,static}`
