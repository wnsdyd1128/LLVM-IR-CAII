...
CAAS 논문은 CA_i 단독으로 스케줄링 아키텍처를 선택했다. 본 프로젝트는 여기에 크로스-코어 간섭과 선점 지연을 추가 반영한 **CAII_i^static으로 CA_i를 대체**하는 것이 목적이다.

순환 의존 해소: CAII 계산에 코어 배치가 필요하고 코어 배치 결정에 CAII가 필요하지만, **n×n 간섭 행렬을 IR 분석 단계에서 코어 배치 없이 미리 계산**해두면 후보 배치가 바뀔 때마다 행렬 합산만 다시 수행하면 되므로 순환이 끊긴다.
...
파이프라인은 2단계로 분리된다. **Stage 1은 코어 배치와 완전히 독립**적으로 1회만 실행되며, Stage 2는 배치 후보를 바꿔가며 반복 실행된다.
...
│    CRPD_i^static  = Σ_{j∈hp(i)} min(UCB_max_i, ECB_j) × BRT │
...
**코어 배치 없이** 모든 태스크 쌍 (i, k)에 대해 n×n 간섭 행렬을 계산한다. CB_k = ECB_k.
...
**Multi-way 간섭:** b ∈ HB_i ∩ CB_j ∩ CB_k 일 때 b가 두 번 카운트되나, j 퇴출 후 재적재 → k 퇴출 후 재적재 시나리오에 대응하는 over-approximation이다. Safe upper bound가 유지된다.
